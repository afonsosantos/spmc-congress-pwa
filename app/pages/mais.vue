...
<template>
  <div class="max-w-2xl mx-auto px-4 pt-6 pb-8 md:px-8 space-y-6">
    ...
    <section v-if="auth.isAuthenticated" class="rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
      ...
      <NuxtLink to="/bilhete" class="inline-flex items-center gap-1.5 mt-3 text-sm font-semibold text-brand-700 dark:text-brand-400">
        <Icon name="ticket" class="w-4 h-4" /> {{ t('more.viewTicket') }}
      </NuxtLink>
    </section>
    <NuxtLink v-else to="/entrar" class="flex items-center justify-between rounded-2xl bg-gold-500 text-white px-5 py-4">
    ...
    <section v-if="infoLinks.length">
      ...
      <div class="rounded-2xl border border-slate-200 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800 overflow-hidden">
        <NuxtLink v-for="l in infoLinks" :key="l.slug" :to="`/info/${l.slug}`" class="flex items-center justify-between px-4 py-3">
    ...
    <section v-if="legalLinks.length">
      ...
      <div class="rounded-2xl border border-slate-200 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800 overflow-hidden">
        <NuxtLink v-for="l in legalLinks" :key="l.slug" :to="`/info/${l.slug}`" class="flex items-center justify-between px-4 py-3">
          <span class="flex items-center gap-3 text-sm font-medium"><Icon :name="l.icon" class="w-4 h-4 text-slate-400" />{{ l.title }}</span>
          <Icon name="chevronRight" class="w-4 h-4 text-slate-300" />
        </NuxtLink>
      </div>
